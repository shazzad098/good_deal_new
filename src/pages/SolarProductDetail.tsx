...
export default function SolarProductDetail() {
  const { id } = useParams();
  useReveal();
  ...
  const product = solarProducts.find((p) => p.id === Number(id));

  if (!product) {
    return <Navigate to="/sustainable-solar-solutions" replace />;
  }
  ...
  return (
    <>
      <nav className="gd-nav" style={{ position: "sticky", top: 0 }}>
        ...
        <Link to="/sustainable-solar-solutions" className="nav-cta" style={{ display: "flex", alignItems: "center", gap: "8px" }}>
          <ArrowLeft size={14} />
          Back to Solutions
        </Link>
      </nav>
