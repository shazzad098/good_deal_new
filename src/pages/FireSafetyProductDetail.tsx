...
import { fireSafetyProducts } from "../data/fireSafetyProducts";
import { useReveal } from "../hooks/useReveal";

export default function FireSafetyProductDetail() {
  const { id } = useParams();
  useReveal();
  ...
  const product = fireSafetyProducts.find((p) => p.id === Number(id));

  if (!product) {
    return <Navigate to="/fire-life-safety-solutions" replace />;
  }
  ...
  return (
    <>
      <nav className="gd-nav" style={{ position: "sticky", top: 0 }}>
        ...
        <Link to="/fire-life-safety-solutions" className="nav-cta" style={{ display: "flex", alignItems: "center", gap: "8px" }}>
          <ArrowLeft size={14} />
          Back to Solutions
        </Link>
      </nav>
            ...
            <div className="product-detail-content reveal" style={{ animationDelay: "0.2s" }}>
              ...
              <div style={{ marginTop: "48px" }}>
                <a href="#contact" className="btn-primary" onClick={(e) => {
                  e.preventDefault();
                  // For now, redirect to home with contact hash or just navigate back to home then open modal
